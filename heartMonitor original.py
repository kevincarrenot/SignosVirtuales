...
class Camera(object):
	...
	def get_frame(self):
		...
		frame = None
		while not frame:
			frame = cv2.cv.QueryFrame(self.cam)
		return frame
...
class FaceDetector(object):
	...
	def get_faces(self, image):
		...
		cv2.CvtColor(image, self.gray, cv2.CV_BGR2GRAY)
		cv2.EqualizeHist(self.gray, self.gray)
		
		# Detect faces
		return cv2.HaarDetectObjects(self.gray, self.cascade, self.storage,
		                            scale_factor = 1.3,
		                            min_neighbors = 2,
...
class Annotator(object):
	...
	def _get_font(self, size=1, weight=1, italic=0):
		return cv2.InitFont(cv2.CV_FONT_HERSHEY_SIMPLEX,
	...
	def draw_bpm(self, frame, bpm):
		...
		cv2.PutText(frame, "%0.0f"%bpm, (x,y), self.large_font_outline, c[1])
		cv2.PutText(frame, "%0.0f"%bpm, (x,y), self.large_font, c[0])
	...
	def draw_phase(self, frame, phase):
		...
		cv2.Circle(frame, (x,y), radius + Annotator.BORDER, c[1], -1)
		cv2.Circle(frame, (x,y), radius, c[0], -1)
	...
	def draw_face(self, frame):
		...
		cv2.Ellipse(frame, (int(x),int(y)), (int(w/2),int(h/2)), 0, 0, 360, c, Annotator.THIN)
	...
	def draw_forehead(self, frame):
		...
		cv2.Rectangle(frame, (int(x),int(y)), (int(x+w),int(y+h)), c, Annotator.THIN)
	...
	def draw_fft(self, frame, fft_data, min_bpm, max_bpm):
		...
		cv2.PolyLine(frame, [line], False, self.get_colour()[0], 3)
		...
		cv2.PutText(frame, text, (x,y), self.small_font_outline, c[1])
		cv2.PutText(frame, text, (x,y), self.small_font, c[0])
		
		# Pulse ring
		r = Annotator.SMALL_PULSE_SIZE
		phase = int(((fft_data[max_bin][1][1] % (2*numpy.pi)) / numpy.pi) * 180)
		cv2.Ellipse(frame, (int(x-(r*1.5)),int(y-r)), (int(r),int(r)), 0, 90, 90-phase, c[1], Annotator.THIN+Annotator.BORDER)
		cv2.Ellipse(frame, (int(x-(r*1.5)),int(y-r)), (int(r),int(r)), 0, 90, 90-phase, c[0], Annotator.THIN)
...
class Program(object):
	...
	def sample_frame(self, frame):
		...
		cv2.SetImageROI(frame, self.face_tracker.get_forehead())
		sample = cv2.Avg(frame)[1]
	...
	def update(self):
		...
		cv2.ShowImage(self.window, frame)
